...
Restart=on-failure
RestartSec=5s
ExecStart=$CORE_GETH_Dir/geth --identity "$IDENTITY_NAME" --classic --datadir $CORE_GETH_DATA_Dir --ethash.dagdir $CORE_GETH_DAG_Dir --http --http.addr 0.0.0.0 --http.port 8545 --http.api eth,web3,net,miner,txpool --syncmode full --miner.etherbase $ETC_MINER_WALLET_ADDRESS
